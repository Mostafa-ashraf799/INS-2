...
export default function AnnouncementSection({ category }: { category: string }) {
  ...
  useEffect(() => {
    setItems(getAnnouncements(category));
    ...
    <div className="flex flex-col h-full">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="p-4 border-b border-dark-700/50 bg-dark-800/30 space-y-3 animate-fade-in">
          ...
          <button
            type="submit"
            className="w-full bg-primary-600 hover:bg-primary-500 text-white py-2 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center gap-2"
          >
            <Megaphone size={16} />
            نشر الإعلان
          </button>
