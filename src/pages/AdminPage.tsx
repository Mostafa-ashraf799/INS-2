...
export default function AdminPage() {
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    e.preventDefault();
    setLoginError('');
    const res = loginUser(adminId, adminPass);
        ...
        <div className="w-full max-w-md">
          ...
          <div className="glass rounded-2xl p-6 gradient-border">
            ...
            <form onSubmit={handleAdminLogin} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full py-3 bg-gradient-to-l from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white rounded-xl font-bold transition-all flex items-center justify-center gap-2"
              >
                <LogIn size={18} />
                دخول لوحة الإدارة
              </button>
            ...
            <div className="mt-4 p-3 rounded-xl bg-dark-800/30 border border-dark-700/50">
              <p className="text-xs text-dark-500 text-center mb-1">بيانات المدير الافتراضية:</p>
              <p className="text-xs text-dark-400 text-center font-mono" dir="ltr">admin@ins.com / admin123</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          {showForm && (
            <form onSubmit={handleAddNews} className="glass rounded-2xl p-6 space-y-4 animate-fade-in">
              ...
              <button
                type="submit"
                className="w-full py-3 bg-gradient-to-l from-primary-600 to-accent-600 hover:from-primary-500 hover:to-accent-500 text-white rounded-xl font-bold transition-all flex items-center justify-center gap-2"
              >
                <Newspaper size={16} /> نشر الخبر
              </button>
            </form>
          )}
                ...
                <tbody>
                  {filteredUsers.map(u => (
                    <tr key={u.id} className="border-t border-dark-700/30 hover:bg-dark-800/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <code className="text-xs bg-dark-800/50 px-2 py-1 rounded text-amber-400" dir="ltr">{u.password}</code>
                      </td>
